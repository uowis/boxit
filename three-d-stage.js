  ...
  class ThreeDStage extends HTMLElement {
    ...
    connectedCallback() {
      if (this._booted) {
        ...
        if (this._renderer) this._resume();
        else this.ready.then(() => this._resume()).catch(() => {});
        return;
      }
      this._booted = true;
    ...
    async _boot() {
      ...
      const fit = () => {
        ...
        const w = Math.round(r.width) || this.clientWidth || 0;
        const h = Math.round(r.height) || this.clientHeight || 0;
        if (!w || !h) return false;
      ...
      this._fit = () => {
        const ok = fit();
        if (ok && this._scene) renderer.render(this._scene, camera);
        else if (!ok) this._scheduleFit();
        return ok;
      };
      fit();
    ...
    /** The host can be unlaid-out at boot (hidden/zero-size iframe) and
     *  get no ResizeObserver notification later — retry until it measures. */
    _scheduleFit() {
      ...
      const tick = () => {
        ...
        if (this._fit()) { this._renderer.setAnimationLoop(this._loop); return; }
        if (++tries > 600) return;
        this._fitPending = true;
      ...
      requestAnimationFrame(tick);
      if (!this._visHook) {
        this._visHook = () => this._scheduleFit();
        document.addEventListener('visibilitychange', this._visHook);
        window.addEventListener('focus', this._visHook);
        window.addEventListener('resize', this._visHook);
        try {
          this._io = new IntersectionObserver(() => this._scheduleFit());
          this._io.observe(this);
        } catch (e) { /* no IO */ }
      }
    }

    /** Idempotent: size to the host, observe it, and run the loop. */
    _resume() {
      if (!this._renderer || !this.isConnected) return;
      this._fit && this._fit();
